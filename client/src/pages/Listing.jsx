          ...
          <div className="flex flex-col max-w-4xl mx-auto p-3 my-7 gap-4">
            ...
            <div className="flex gap-4">
              <p className="bg-red-900 w-full max-w-[200px] text-white text-center p-1 rounded-md">
                {listing.type === "rent" ? "Сдается в аренду" : "Продается"}
              </p>
              {listing.offer && (
                <p className="bg-green-900 w-full max-w-[200px] text-white text-center p-1 rounded-md">
                  Скидка {+listing.regularPrice - +listing.discountPrice}{" "}
                  <span className="text-xs">
                    <a className="text-base">{"\u20bd"}</a>
                  </span>
                </p>
              )}
            </div>
            ...
            <ul className="text-green-900 font-semibold text-sm flex flex-wrap items-center gap-4 sm:gap-6">
              ...
              <li className="flex items-center gap-1 whitespace-nowrap ">
                ...
                {listing.bathrooms > 1
                  ? `${listing.bathrooms} Уборных `
                  : `${listing.bathrooms} Уборная `}
              </li>
              <li className="flex items-center gap-1 whitespace-nowrap ">
              ...
              <li className="flex items-center gap-1 whitespace-nowrap ">
                ...
                {listing.furnished ? "Мебель" : "Без мебели"}
              </li>
            </ul>
            {currentUser && listing.userRef === currentUser._id && !contact && (
              <button
                onClick={() => setContact(true)}
                className=" mt-4 bg-green-700 text-white p-3 rounded-lg uppercase min-w-full hover:bg-green-800 hover:shadow-lg transition m-auto"
              >
