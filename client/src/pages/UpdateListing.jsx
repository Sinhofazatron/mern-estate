  ...
  const handleImageSubmit = () => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch(() => {
          setImageUploadError("Ошибка загрузки изображения (максимум 2мб)");
          setUploading(false);
        });
    } else {
      setImageUploadError("Вы можете загрузить только 6 изображений");
      setUploading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        <div className="flex flex-col gap-4 flex-1">
          ...
          <textarea
            ...
            className="border p-3 rounded-lg min-h-[130px]"
            id="description"
            required
            onChange={handleChange}
        ...
        <div className="flex flex-col flex-1 gap-4">
          <p className="font-semibold">
            ...
            <span className="font-normal text-gray-600 ml-2">
              Первое изображение будет обложка (максимум 6)
            </span>
          </p>
